...
const Tabs: React.FunctionComponent<Props> = (props: Props) => {
  const {activeTab, onActiveTabHandler, film} = props;
  return (
    <div className="movie-card__desc">
      <nav className="movie-nav movie-card__nav">
        <ul className="movie-nav__list">
          {TAB_LIST.map((navItem) => {
            return (
              <li key={navItem} className={`movie-nav__item ` + (navItem === activeTab ? `movie-nav__item--active` : ``)}>
                <a
                  href="#"
                  className="movie-nav__link"
                  onClick={(evt) => {
                    evt.preventDefault();
                    onActiveTabHandler(navItem);
                  }}
                >
                  {navItem}
                </a>
      ...
      {activeTab === TAB_NAME.reviews && (<div className="movie-card__reviews movie-card__row">
